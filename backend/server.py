...
@app.get("/")
async def root():
    return {"message": "Poromet API is running", "status": "healthy"}

@app.get("/api/health")
async def health_check():
    return {"status": "healthy", "message": "Poromet API is running"}

@app.get("/api/histogram/{output_dir}")
async def get_histogram(output_dir: str):
    """Serve the histogram image for a specific analysis"""
    try:
        print(f"\n=== Histogram Request ===")
        ...
        if output_dir not in analysis_results:
            print(f"Error: Output dir {output_dir} not found in analysis_results")
            return JSONResponse(
                status_code=404,
        ...
        output_dir_path = analysis_results[output_dir]
        hist_path = os.path.join(output_dir_path, "pore_size_distribution.png")
        print(f"Looking for histogram at: {hist_path}")
        
        if not os.path.exists(hist_path):
            print(f"Error: Histogram not found at {hist_path}")
            if os.path.exists(output_dir_path):
                print(f"Files in directory {output_dir_path}:")
            ...
            return JSONResponse(
                status_code=404,
                content={"detail": f"Histogram not found at {hist_path}"},
                headers={"Access-Control-Allow-Origin": "*"}
            )
            
        print("Serving histogram file")
        return FileResponse(
            path=hist_path,
        ...
        logger.error(f"HTTPException in get_histogram: {str(he)}")
        raise
    except Exception as e:
        logger.error(f"Unexpected error in get_histogram: {str(e)}\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="Failed to serve histogram")
